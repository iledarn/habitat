    ...
    pub fn reconfigure(&mut self, census_list: &CensusList) {
        ...
        let mut service_config =
            match ServiceConfig::new(&sg, &self.package, census_list, gconfig().bind()) {
                Ok(sc) => sc,
                Err(e) => {
                    outputln!("Error generating Service Configuration; not reconfiguring: {}",
                              e);
                    return;
                }
            };
        ...
        match service_config.write(&self.package) {
            Ok(true) => self.needs_restart = true,
            Ok(false) => {}
            Err(e) => outputln!("Failed to write service configuration: {}", e),
        }
